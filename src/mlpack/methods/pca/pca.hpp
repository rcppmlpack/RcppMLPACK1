...
public:
	...
	void Apply(const arma::mat& data, arma::mat& transformedData,
			arma::vec& eigVal, arma::mat& coeff) const {
		...
		if (scaleData) {
			...
			arma::vec stdDev = arma::stddev(centeredData, 0,
	...
	double Apply(arma::mat& data, const double varRetained) const {
		...
		if (varRetained < 0)
			Rcpp::Rcerr << "PCA::Apply(): varRetained (" << varRetained
					<< ") must be " << "greater than or equal to 0." << std::endl;
		if (varRetained > 1)
			Rcpp::Rcerr << "PCA::Apply(): varRetained (" << varRetained
					<< ") should be " << "less than or equal to 1." << std::endl;
	...
	double Apply(arma::mat& data, const size_t newDimension) const {
		// Parameter validation.
		if (newDimension == 0)
			Rcpp::Rcerr << "PCA::Apply(): newDimension (" << newDimension
	 ...
	 */
	double Apply(const arma::mat& data, arma::mat& transformedData,
